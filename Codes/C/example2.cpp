...
int main(int argc, char* argv[])
{
    ...
    //Declare a output fstream (file stream) variable, output_file, and open a new file (or overwrite existing) with a name
    ofstream output_file;
    output_file.open("results.csv");

    //If file is not open, exit
    if (!output_file.is_open()) {
        cerr << "File could not be opened, exitting..." << endl;
        exit(1);
    }

    output_file << low << ", " << high << endl;
    while (low < high) {
        flag = 0;
    ...
    //Must flush (commit all writes) and close the file at the end
    output_file.flush();
    output_file.close();
